...
class CityListFavoritePresenterTest {
    ...
    @Test
    fun `test filter when filter apply ma then filter result without non-favorite`() {
        ...
        val result = cityViewList.filter { cityView ->
            cityView.cityName.contains(FILTER_QUERY_MADRID, true) &&
                    cityView.favoriteImg == R.drawable.ic_favorite_enable
        }
